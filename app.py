...
def parse_date(series):
    """
    Normalize timestamps to UTC then strip timezone
    to avoid tz-aware / tz-naive comparison issues.
    """
...
def load_data():
    try:
        ...
        if not rev_df.empty:
            rev_df["amount"] = pd.to_numeric(rev_df.get("amount"), errors="coerce").fillna(0)

            rev_date_source = rev_df["revenueDate"] if "revenueDate" in rev_df.columns else rev_df.get("date")
            rev_df["date"] = parse_date(rev_date_source)
        ...
        if not exp_df.empty:
            ...
            if date_col:
                exp_df["date"] = parse_date(exp_df[date_col])
            else:
                exp_df["date"] = pd.NaT
        ...
        if not par_df.empty:
            ...
            if date_col:
                par_df["date"] = parse_date(par_df[date_col])
            else:
                par_df["date"] = pd.NaT
...
if section == "📦 Parcel Analytics":
    ...
    if par_filt.empty:
        ...
        st.markdown("Dashboard loaded successfully.")
    ...
    total_expenses = exp_filt["total_expense"].sum() if not exp_filt.empty else 0
    net_profit = total_revenue - total_expenses
    profit_margin = (net_profit / total_revenue * 100) if total_revenue > 0 else 0

    k1, k2, k3, k4 = st.columns(4)

    k1.markdown(kpi_card("Total Revenue", f"KES {total_revenue:,.0f}", "revenue-card"), unsafe_allow_html=True)
    k2.markdown(kpi_card("Total Expenses", f"KES {total_expenses:,.0f}", "expense-card"), unsafe_allow_html=True)
    k3.markdown(kpi_card("Net Profit", f"KES {net_profit:,.0f}", "profit-card"), unsafe_allow_html=True)
    k4.markdown(kpi_card("Profit Margin", f"{profit_margin:.1f}%", "profit-card"), unsafe_allow_html=True)
